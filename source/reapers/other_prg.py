...
class OtherProg(Reaper, OutReader):
    ...
    def run(self):
        ...
        Thread(target=self.out_reader, args=[prg, self.splitter], daemon=True).start()
        ...
        while prg.poll() is None:
            ...
            try:
                ...
                if self.percent_type == 'int':
                    percent = int(self.output[self.percent_index])
                elif self.percent_type == 'l_first':
                    first, second = self.output[self.percent_index].split(self.percent_del)
                ...
                self.update_signal.emit(percent, '', f'{localize.saving} - {self.output[self.name_index]}...', False)
