...
class QuickOpen(QProcessList):
    ...
    def find_reaper(self):
        ...
        # Check on video formats
            ...
            self.q_connect(self.ffmpeg, self.file_name, header='Convert...')
        ...
        # Check on Hrust archive
            ...
            if self.head == b'\x37\xBD\x37\x4D':  # 7Ѕ7M, PopCap PAK
                ...
                if 'data.000.pak' in self.file_name:  # 1242
                    self.qbms.script_name = f"{self.root_dir}/data/scripts/1242.bms"
                    self.q_connect(self.qbms, self.file_name)
                elif 'azangara' in self.file_name.lower():  # Azangara
                ...
                if 'Content/Paks' in self.file_name:  # Unreal Engine 4
                    self.q_connect(self.unreal, self.file_name)
