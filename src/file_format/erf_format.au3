			...
			If $iVer = '0x56312E30' Then
				;MsgBox($MB_SYSTEMMODAL, $tMessage, "v1")
					FileSetPos ($iFile, 16, 0)
						...
						$iFileCount = Dec(StringTrimLeft (_BinaryToInt32($iFileCount), 2))
					FileSetPos ($iFile, 24, 0)
						$iOff2Lst = FileRead ($iFile, 4)
						$iOff2Lst = Dec(StringTrimLeft (_BinaryToInt32($iOff2Lst), 2))
					FileSetPos ($iFile, 28, 0)
						$iERFLst = FileRead ($iFile, 4)
						$iERFLst = Dec(StringTrimLeft (_BinaryToInt32($iERFLst), 2))
					...
					ProgressOn('', $tWtSaving, '', (@DesktopWidth/2)-150, (@DesktopHeight/2)-62, 18)
						For $j = 1 to $iFileCount
							$data1 = FileRead ($iFile, 4)
								$data1 = Dec(StringTrimLeft (_BinaryToInt32($data1), 2))
							$data2 = FileRead ($iFile, 4)
								$data2 = Dec(StringTrimLeft (_BinaryToInt32($data2), 2))
							$c = FileGetPos($iFile)
						...
						ProgressOn('', $tWtSaving, '', (@DesktopWidth/2)-150, (@DesktopHeight/2)-62, 18)
							For $i = 0 to $iFileCount
								...
								$iOffs = FileRead ($iFile, 4)
									$iOffs = Dec(StringTrimLeft (_BinaryToInt32($iOffs), 2))
								$iLong = FileRead ($iFile, 4)
									$iLong = Dec(StringTrimLeft (_BinaryToInt32($iLong), 2))
								$c = FileGetPos($iFile)
