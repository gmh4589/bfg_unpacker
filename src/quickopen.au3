...
Func QuickPAK($sFileName); TODO: �������� GAUP(Quake � ��.)!!!	
	...
	Switch $iFind
		...
		Case Else
			...
			ElseIf StringInStr($iDir, 'Sacred') Then
				_MsgBox(0, $tMessage, '�nly textures support for Sacred!')
			Else
				_MsgBox(0, $tMessage, $tNotice3)
				GUICtrlSetData($iEdit, $tNotice3 & @CRLF, 1)
				;_QuickBMSRun('', @ScriptDir & "\data\wcx\gaup_pro.wcx ", $sFileName)
...
Func QuickBIN($sFileName)
	;PASS
EndFunc

Func QuickCache($sFileName)
	;PASS
EndFunc

Func QuickWAD($sFileName)
	;PASS
EndFunc

Func QuickEXE($sFileName)
	;PASS
EndFunc

Func QuickDZIP($sFileName)
	;PASS
EndFunc

Func QuickPGZ($sFileName)
	;PASS
EndFunc

Func QuickARK($sFileName)
	;PASS
EndFunc

Func QuickARC($sFileName)
	;PASS
EndFunc

Func QuickBIF($sFileName)
	;PASS
EndFunc

Func QuickBIG($sFileName)
	;PASS
EndFunc

Func QuickFAT($sFileName)
	;PASS
EndFunc

Func QuickKEY($sFileName)
	;PASS
EndFunc

Func QuickVOL($sFileName)
	;PASS
EndFunc

Func QuickPKG($sFileName)
...
Func QuickSPK($sFileName)
	;PASS Of Orcs and Men
EndFunc

Func QuickVID($sFileName)
	;PASS
EndFunc

Func QuickVIV($sFileName)
	;PASS
EndFunc

Func QuickMOD($sFileName)
	;PASS
EndFunc

Func QuickBundle($sFileName)
	Switch _headRead($sFileName, 3, 16)
		Case '0x504F54'
			_Engine('_RedEngine', $sFileName)
		Case Else
